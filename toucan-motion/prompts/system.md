...
**C. Dive into a node = a scene you build INSIDE it, by nested reveal.** The camera pushes INTO node `01` and STAYS inside it: the node becomes the container (a stylized browser/app/diagram **inset card** — see §4) that fills the frame, and the scene plays out in its interior. Inside, sub-steps appear **one at a time, each spawning FROM the previous element's position** — A appears, B emerges out of A, C out of B — nested *within* the node's space, never laid out flat side by side like a second graph. Exactly one sub-step is focal at a time (§5): as each arrives the camera frames it and the previous one is pushed back (dimmed / blurred / scaled) or removed. **Every element — node, sub-step, label, cursor — has an explicit appear time AND disappear time in `render(ms)`;** when a beat ends, its element is driven to opacity 0 (or out of frame), so nothing from a finished sub-step lingers. Only when node `01`'s scene FULLY completes does the camera move on: a lateral move to `02`, or a brief pull-back to re-orient on the graph (with `01` now marked done) before diving into `02`. The graph is the spine you re-orient against between dives — a beat, not a flat hold.
...
- **Focal staging — isolate, don't just center.** At every beat exactly ONE element owns the frame; everything else is *actively suppressed*, not merely pushed off to the side. Drive a per-element "focus weight" from `render(ms)` (a function of the active beat and each element's distance from the focal one) that: lowers non-focal **opacity**, adds `filter: blur()` to non-focal elements (0 on the focal one — set from inside `render(ms)`, never CSS), and/or **scales them down**. Give the active element subtle depth so it lifts off the dimmed background: a slight scale-up (~1.03–1.06) plus one soft shadow. There must be NO frame where all nodes/sub-steps sit at equal opacity and equal sharpness — that reads as a flat diagram, not a directed shot.
...
- [ ] The Overview BUILDS one node at a time (each spawning from the previous); dives push INTO a node and stay inside it, with sub-steps nesting one at a time out of the previous element — never a flat graph, never flat side-by-side sub-steps.
- [ ] Every element has an explicit appear AND disappear time in `render(ms)`; nothing from a finished sub-step or node stays visible after the camera moves on.
- [ ] At no point are all stages/nodes visible at equal weight. Exactly one element owns the frame at any moment; everything else is dimmed, blurred, or scaled back.
