...
public class CodeBlockAnimationGenerator implements AnimationGenerator {
    ...
    private String buildUserPrompt(String description) {
        return "Concept to explain as an animated code block:\n\n"
                + description
                + "\n\nProduce ONE complete, self-contained HTML document that animates THIS concept, "
                + "matching the architecture and visual calibre of the reference below. Write fresh code, "
                + "annotations, filename, and styling appropriate to this concept — do NOT reuse the "
                + "reference's Fibonacci content, and do not include any footer text.\n\n"
                + "===== REFERENCE: expected structure & quality (do not copy verbatim) =====\n"
                + referenceExemplar()
                + "\n===== END REFERENCE =====\n\n"
                + "Output only the final HTML document, nothing before or after it.";
    }

    /** The hand-built stub doubles as the gold-standard example we show the model. */
    private static String referenceExemplar() {
        return STUB_TEMPLATE
                .replace(
                        "__DESCRIPTION__",
                        "Memoize a recursive Fibonacci function to cache repeated subproblems")
                .replace("Generated by Toucan — preview stub (no model key configured)", "");
    }
    ...
    private static final String SYSTEM_PROMPT =
            ...
            that animates an explanation of that concept as a code block.

            ARCHITECTURE — build it this way:
            ...
            - When a line finishes typing, highlight it (a left accent bar + a subtle background) \
            and reveal its annotation in sync; clear the previous line's highlight first.
            - After the last step, hold briefly, then gently restart the loop.
            ...
            network calls. Use a system monospace stack for code and a system sans-serif stack for \
            prose.
            ...
            - Autoplay on load, run the full sequence, then loop gently. No user interaction required.
            - The whole animation must fit inside a 1280x720 viewport with NO scrolling, and scale \
            down cleanly on smaller screens.

            VISUAL QUALITY BAR:
            - Modern, clean dark theme. An editor surface with window chrome (three traffic-light \
            dots) and a filename tab. Line numbers down the gutter. Tasteful per-token syntax \
            coloring (keywords, strings, numbers, comments, function names) via classes or inline \
            styles.
            - Smooth, eased transitions. Rounded corners, soft shadows, comfortable spacing, \
            readable sizes (code around 15px, line-height around 1.8).
            - Annotations fade or slide in and clearly relate to the highlighted line. A short \
            title or kicker at the top is welcome.
            - Pace the typing so it is legible (not too fast) and hold each annotation long enough \
            to read. Never generic, cramped, or janky.

            A reference document showing the expected structure and calibre is included in the user \
            message. Match its architecture and quality, but write FRESH code, copy, and styling for \
            the requested concept — do not reuse its example content. Return only the HTML document.""";
